...
const QrScanner = () => {
    ...
    const decodeQrCode = (img) => {
        ...
        qr.callback = (error, result) => {
            if (error || !result) {
                console.error("QR code decoding error:", error || "No result");
...
const QrScanner = () => {
    ...
    const decodeQrCode = (img) => {
        ...
        if (qrCodeResult) {
            setQrResult(qrCodeResult.data); // Store the plain data from the QR code
            setDialogOpen(true);
        } else {
            console.error("QR code decoding error: No result found");
            setQrResult("Error: QR code not found. Please try a clearer or higher resolution image.");
            setDialogOpen(true);
        }
    ...
    const handleEdit = () => {
        if (qrResult) {
            navigate(`/admin-dashboard/update-equipment/update-equipment/${qrResult}`);
        }
    };
    ...
    return (
        ...
        >
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
                ...
                <DialogContent dividers>
                    {qrResult ? (
                        <Typography>{qrResult}</Typography>
                    ) : (
                        <Typography>Error: Unable to read the QR code.</Typography>
                    )}
                    <Box textAlign="center" mt={3}>
                        <Button onClick={() => setDialogOpen(false)} color="primary" variant="contained">
                            Close
